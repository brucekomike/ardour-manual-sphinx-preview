...
commands have their own path */select*. This means that for the selected
...
1.  Single strip control surfaces. Using */access_action
    Editor/select-next-route* or */access_action
    Editor/select-prev-route* to step through the mixer strips.
2.  Using a \"Super strip\" section of knobs to control parts of the
    strip that are changed less often such as polarity, sends or plugin
    parameters.
...
\"A\" may select strip 4 and use a selected controller to make changes
to that strip. User \"B\" may subsequently select strip 7 to make
changes on. This leaves user \"A\" making changes to strip 7 which they
did not choose.
